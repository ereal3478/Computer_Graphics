...
MagFilter cur_mag_filter = Nearest;
...
MinFilter cur_min_filter = Nearest_Mipmap_Linear;
...
struct Light
{
	int mode = 0;// 0:directional   1:point   2:spot
	Vector3 pos[3] = { Vector3(1,1,1),Vector3(0,2,1),Vector3(0,0,2) };
	float cutoff = 30;// dergee
	Vector3 diffuse_intensity[3] = { Vector3(1, 1, 1),Vector3(1, 1, 1),Vector3(1, 1, 1) };
	float shininess = 64;
};
Light light;
...
void RenderScene(int per_vertex_or_per_pixel) {	
	...
	Matrix4 MVP, ModelView, Viewing;

	MVP = project_matrix * view_matrix * T * R * S;
	ModelView = view_matrix * T * R * S;
	Viewing = view_matrix;

	// use uniform to send mvp to vertex shader
	glUniformMatrix4fv(uniform.iLocMVP, 1, GL_FALSE, MVP.getTranspose());
	glUniformMatrix4fv(uniform.iLocModelView, 1, GL_FALSE, ModelView.getTranspose());
	glUniformMatrix4fv(uniform.iLocViewing, 1, GL_FALSE, Viewing.getTranspose());
	glUniform1i(uniform.iLocLightMode, light.mode);
	glUniform3fv(uniform.iLocLightPos, 3, &light.pos[0][0]);
	glUniform1f(uniform.iLocLightCutoff, light.cutoff);
	glUniform3fv(uniform.iLocLightDiffuse, 3, &light.diffuse_intensity[0][0]);
	glUniform1f(uniform.iLocLightShininess, light.shininess);
	glUniform1i(uniform.iLocIsPerPix, per_vertex_or_per_pixel);
	...
	for (int i = 0; i < models[cur_idx].shapes.size(); i++) 
	{
		...
		glUniform1f(uniform.iLocXoffset, models[cur_idx].shapes[i].material.offsets[models[cur_idx].cur_eye_offset_idx].x);
		glUniform1f(uniform.iLocYoffset, models[cur_idx].shapes[i].material.offsets[models[cur_idx].cur_eye_offset_idx].y);
		...
		if (cur_mag_filter == Nearest)
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
		else
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);

		if (cur_min_filter == Nearest_Mipmap_Linear)
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	if (action == GLFW_PRESS) {
		...
		case GLFW_KEY_L:
			light.mode = (light.mode + 1) % 3;
			if (light.mode == 0)
				cout << "current light mode: Directional light" << '\n';
			else if (light.mode == 1)
				cout << "current light mode: Point light" << '\n';
			else if (light.mode == 2)
				cout << "current light mode: Spot light" << '\n';
			break;
		...
		case GLFW_KEY_G:
			cur_mag_filter = (cur_mag_filter == Nearest) ? Linear : Nearest;
			break;
		case GLFW_KEY_B:
			cur_min_filter = (cur_min_filter == Nearest_Mipmap_Linear) ? Linear_Mipmap_Linear : Nearest_Mipmap_Linear;
			break;
		case GLFW_KEY_LEFT:
...
void scroll_callback(GLFWwindow* window, double xoffset, double yoffset)
{
	...
	case LightEdit:
		if (light.mode == 0 || light.mode == 1) {
			light.diffuse_intensity[light.mode] += Vector3(yoffset * 0.1, yoffset * 0.1, yoffset * 0.1);
			//cout << "light.diffuse_intensity[light.mode]: " << light.diffuse_intensity[light.mode] << '\n';
		}
		else if (light.mode == 2) {
			light.cutoff += yoffset;
			//cout << "light.cutoff: " << light.cutoff << '\n';
		}
		break;

	case ShininessEdit:
		light.shininess += yoffset;
		//cout << "light.shininess: " << light.shininess << '\n';
		break;
...
static void cursor_pos_callback(GLFWwindow* window, double xpos, double ypos)
{
	if (mouse_pressed) {
		if (starting_press_x < 0 || starting_press_y < 0) {
			...
			case LightEdit:
				light.pos[light.mode].x -= diff_x * 0.01;
				light.pos[light.mode].y -= diff_y * (-0.01);
				break;
...
void setUniformVariables()
{
	...
	uniform.iLocModelView = glGetUniformLocation(program, "ModelView");
	uniform.iLocViewing = glGetUniformLocation(program, "Viewing");
	uniform.iLocKa = glGetUniformLocation(program, "material.Ka");
	uniform.iLocKd = glGetUniformLocation(program, "material.Kd");
	uniform.iLocKs = glGetUniformLocation(program, "material.Ks");
	uniform.iLocIsPerPix = glGetUniformLocation(program, "isPerPixel");
	uniform.iLocLightMode = glGetUniformLocation(program, "light.mode");
	uniform.iLocLightPos = glGetUniformLocation(program, "light.pos");
	uniform.iLocLightCutoff = glGetUniformLocation(program, "light.cutoff");
	uniform.iLocLightDiffuse = glGetUniformLocation(program, "light.diffuse_intensity");
	uniform.iLocLightShininess = glGetUniformLocation(program, "light.shininess");

	// [TODO] Get uniform location of texture
	uniform.iLocXoffset = glGetUniformLocation(program, "x_offset");
	uniform.iLocYoffset = glGetUniformLocation(program, "y_offset");
...
int main(int argc, char **argv)
{
    ...
    // create window
	GLFWwindow* window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "111062655 HW3", NULL, NULL);
    if (window == NULL)
    {
